...
## A continuous shower of sparks falling off the streak plate while a flare
...
##
## Each spark spawns at a random point along the plate's bottom edge, across
## its entire length, and drops under a light gravity with a little sideways
## drift -- embers shaken off the underside of the text.
##
...
## Initial downward speed; gravity then pulls each spark faster as it falls.
const BASE_SPEED := 40.0
const SPEED_PER_TIER := 9.0
const GRAVITY := 160.0
## Max sideways speed either way, so the shower isn't a row of straight lines.
const DRIFT := 14.0
const BASE_RADIUS := 5.0
const RADIUS_PER_TIER := 0.4
...
## The last stretch of a burst: spawning stops and the whole spray dims out, so
## every spark is gone by the burst's deadline instead of outliving it.
const END_FADE := 0.25
...
## Whole-spray brightness, ramping to zero over END_FADE before the deadline.
var _burst_alpha: float = 1.0

func _ready() -> void:
	set_process(false)

## Called on every landing that extends the streak. `hold_seconds` is a hard
## deadline: by then every spark has faded out. Takes the *longest* outstanding
## request rather than resetting to it, so a burst already mid-flight is never
## cut short by a smaller one.
...
func _spawn_one() -> void:
	...
	var spark := Spark.new()
	# Just inside the bottom edge, so each spark emerges from under the plate
	# rather than popping into existence below it.
	spark.pos = Vector2(
		randf_range(-plate_size.x / 2.0, plate_size.x / 2.0),
		plate_size.y / 2.0 - randf_range(0.0, 4.0))
	spark.vel = Vector2(randf_range(-DRIFT, DRIFT), speed * randf_range(0.6, 1.0))
...
func _draw() -> void:
	for spark in _sparks:
		...
		var col := color
		col.a *= fade * spark.base_alpha * _burst_alpha
